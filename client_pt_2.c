...
void setIPAndTCPHeaders(struct ip* ip_header, struct tcpheader* tcp_header, const struct ServerConfig* config, int destinationPort) {

    ip_header->ip_hl = 5;  // Header length (5 words)
    ip_header->ip_v = 4;   // IPv4
    ip_header->ip_tos = 0; // Type of Service
    ip_header->ip_len = sizeof(struct ip) + sizeof(struct tcpheader); // Total length
    // ip_header->ip_len = sizeof(struct ip) + sizeof(struct tcphdr); // Total length
    ...
    // Use the actual IP address of your network interface
    ...
    ip_header->ip_sum = checksum((unsigned short*)ip_header, sizeof(*ip_header) / 2);
    tcp_header->th_sum = checksum((unsigned short*)tcp_header, sizeof(*tcp_header) / 2);
}

int sendTCP(const struct ServerConfig* config, int destinationPort) {
    // Create a raw socket
    ...
    // Set IP and TCP headers
    struct ip ip_header;
    // struct tcphdr tcp_header;
    struct tcpheader tcp_header;

    setIPAndTCPHeaders(&ip_header, &tcp_header, config, destinationPort);
    ...
    if (sendto(raw_socket, packet, ip_header.ip_len, 0, (struct sockaddr*)&dest, sizeof(dest)) < 0) {
    ...
    // Wait for a short duration to allow time for a potential response
    sleep(1);

    // Read the response
    char response[2048];
    ssize_t bytesRead = recv(raw_socket, response, sizeof(response), 0);
    if (bytesRead > 0) {
...
int main() {
    ...
    //It should send correctly but I need look on wire shark to see if RST packet is sent back


    printf("Dest port head: %d\n", config.destinationPortTCPHeadSYN);
    int result = sendTCP(&config, config.destinationPortTCPHeadSYN);
    if (result == 0) {
        printf("Head SYN packet sent to the server successfully.\n");
    } else {
        printf("Failed to send the head SYN packet.\n");
    }
...
//gcc -o client2 client_pt_2.c -lcjson -I/usr/local/opt/cjson/include -L/usr/local/opt/cjson/lib
